...
-- Date
-- 此文件由[BabeLua]插件自动生成
local AgeCell = class("AgeCell", ccui.Layout)
function AgeCell:ctor(index)
...
function AgeCell:touchEvent(sender, eventType)
...
function AgeCell:getIndex()
    return self.index
end
function AgeCell:updateStatus(index)
    local isSelf = false
    local selfIndex=bole:getUserDataByKey("age")
    if index then
        selfIndex=index
    end
