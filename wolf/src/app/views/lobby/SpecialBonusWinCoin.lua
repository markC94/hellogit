...
function SpecialBonusWinCoin:startBonusAnim(auto)
    self.isFlying = true
    ...
    if not auto then
        ...
        bonusNode:setPosition(0, 60)
        performWithDelay(self, function()
            self.act_icon = sp.SkeletonAnimation:create("common_act/JBbaozha.json", "common_act/JBbaozha.atlas")
            self.act_icon:setAnimation(0, "animation", false)
            self:addChild(self.act_icon)
            self.act_icon:setPosition(0, 60)
        end , 0.1)
        self:flowerCoin()
    end
...
function SpecialBonusWinCoin:flowerCoin()
    ...
    launchFunc = function()
        ...
        local decreaseRate = 0.8 - timeCount * 0.06
        for i = 1, num do
    ...
    launchOneCoin = function(decreaseRate)
        local coinNode = sp.SkeletonAnimation:create("util_act/coin_turnd.json", "util_act/coin_turnd.atlas", math.random(6, 12) / 10)
        ...
        local scale=1
        if math.abs(newX)>50 then
            scale=1- math.abs(newX)*0.001
            print("------------------------------------scale="..scale)
            if scale<0.7 then
                scale=0.7
            end
...
--        print("------------------------------------flyLen="..newX)
        local scaleAct = cc.ScaleTo:create(0.2,scale)
        ...
        coinNode:runAction(cc.Sequence:create(cc.Spawn:create(fadeIn, easeInAct,scaleAct), easeOutAct, easeInOutAct, fadeOutAct, endCallAct))
    end
    ...
    flyEndFunc = function()
        if launchCount == 0 and stopLaunch and self.isAlive and not isRemoveSelf then
            isRemoveSelf = true
