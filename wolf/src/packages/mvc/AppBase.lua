...
function AppBase:createView(name,path)
    for _, root in ipairs(self.configs_.viewsRoot) do
        ...
        if status and(t == "table" or t == "userdata") then
            if path then
                return view:create({name=name,path=path})
            else
...
--function AppBase:getViewByName(viewName)
--    return cc.loadLua(string.format("app.views.%s", viewName))
--end

function AppBase:getConfigByName(name, id, key)
